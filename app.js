(() => {
  ...
  function isCompactMode() {
    return sheet.classList.contains('is-exporting') || sheet.classList.contains('is-printing');
  }

  function screenMinimumHeight(el) {
    if (el.classList.contains('plain-textarea')) return 66;
    if (el.classList.contains('h-1')) return 40;
    return 74;
  }

  function fitText(el) {
    if (!isCompactMode()) {
      /* Writing view: keep readable text and expand the box instead of shrinking it. */
      el.style.fontSize = '';
      el.style.height = 'auto';
      el.style.height = `${Math.max(screenMinimumHeight(el), el.scrollHeight)}px`;
      return;
    }

    /* A4 view: restore the fixed print height, then shrink only when required. */
    const max = 9.8;
    const min = 7.0;
    let size = max;
    el.style.height = '';
